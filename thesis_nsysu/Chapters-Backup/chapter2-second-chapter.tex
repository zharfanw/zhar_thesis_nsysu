...
Basic Information of Antenna, including the antenna type, frequency band, and antenna parameters.
- On general, Antenna Model is simplified with Far-Field Model. 

\subsection{Antenna Modeling}
% In this section, antenna propagation is modeled under free-space conditions, where a fixed transmitting and receiving antenna are positioned at a specific distance $r$. Given that the distance between them is sufficiently large to justify the spherical wave approximation, the transmitted sinusoidal signal $\cos 2\pi f$ propagates with an electric field $E(f, t, \boldsymbol{u})$ characterized by:
% \begin{equation}
% E(f, t, (r, \theta, \psi)) = \frac{\alpha_s(\theta, \psi, f)}{r} \cos 2\pi f(t - r/c)
% \end{equation}

% Consequently, the received waveform corresponding to the transmitted signal can be represented as:
% \begin{equation}
% E(f, t, \boldsymbol{u}) = \frac{\alpha(\theta, \psi, f)}{r} \cos 2\pi f(t - r/c)
% \end{equation}
% \cite{Tse2005}
...
% \subsection{Radiation Pattern of Antenna}

% \section{Metasurface Antenna Lens Design}
% \subsection{FSS/unitcell design}
%     Explain about FSS/unitcell design as general, including the unitcell design and simulation setup.
% \subsection{Antenna Lens Design}
%     Arrange of the unitcell to form the antenna lens, and explain about the antenna lens design as general, including the phase compensation and simulation setup.
% \subsection{Radiation Pattern of Antenna with Lens Enhanced}
%     Explain about the radiation pattern of antenna lens as general, including the simulation setup and evaluation.
% \section{Setup of Simulation Environment}
%     Explanation about general Simulation setup, including the simulation software, simulation environment, and simulation workflow.
%     \subsection{Simulation Setup with SIONNA-RT}
%     \subsection{Simulation Setup with HFSS-SBR+}
%     \subsection{Simulation Setup with HFSS-Hybrid}
% \section{Comparison Simulation and Evaluation}
%     \subsection{Channel Evaluation}

% \section{Antenna Theory}

% \subsection{Radiation Pattern}
% Explanation about Radiation pattern as general

% \subsubsection{Far Field of a Transmitting Antenna}

% \subsubsection{Model of Receiver Antenna}
% Please create another title for this title. Then Explain of Model of Receiver Antenna as general

% \section{Ray Tracing and Propagation Model}
% Explain Method that used in this thesis to evaluate the wireless channel, including ray tracing and propagation model.

% \subsection{General Propagation Path}
% Explanation about general propagation path, including direct path, reflected path, and diffracted path.
% \subsubsection{Reflection and Refraction}

% \subsubsection{Difraction}
...
% The radiation pattern describes the spatial variation of the electromagnetic power radiated by an antenna as a function of direction. It is commonly represented in spherical coordinates by the elevation angle $\theta$ and azimuth angle $\phi$, while the observation distance is kept constant. Depending on the quantity being observed, the pattern may be expressed as an electric-field pattern, a power pattern, a directivity pattern, or a gain pattern. In antenna analysis, the normalized radiation pattern is particularly useful because it emphasizes the angular shape of the radiation independently of its absolute magnitude~\cite{ConstantineABalanis2016}.

% The electromagnetic region surrounding an antenna can be divided into the reactive near-field, radiating near-field (Fresnel), and far-field (Fraunhofer) regions. For an antenna with a maximum overall dimension $D$ that is large compared with the wavelength $\lambda$, the far-field region is commonly approximated by
% \begin{equation}
%     r \geq \frac{2D^2}{\lambda},
% \end{equation}
% where $r$ is the distance from the antenna to the observation point. In this region, the electric and magnetic fields are transverse to the direction of propagation, their angular distributions are essentially independent of $r$, and their magnitudes decrease approximately in proportion to $1/r$. Consequently, the radiated power density decreases approximately as $1/r^2$. These properties allow the antenna pattern to be evaluated only as a function of $\theta$ and $\phi$ without including the observation distance in the normalized pattern.

% If $U(\theta,\phi)$ denotes the radiation intensity in a given direction and $U_{\max}$ is its maximum value, the normalized power pattern can be written as
% \begin{equation}
%     P_{\mathrm{n}}(\theta,\phi)
%     =
%     \frac{U(\theta,\phi)}{U_{\max}},
% \end{equation}
% or expressed in decibels as
% \begin{equation}
%     P_{\mathrm{n,dB}}(\theta,\phi)
%     =
%     10\log_{10}\left[P_{\mathrm{n}}(\theta,\phi)\right].
% \end{equation}
% The resulting pattern consists of radiation lobes separated by nulls. The main lobe indicates the direction of maximum radiation, whereas side lobes represent radiation in undesired directions. The half-power beamwidth (HPBW) is the angular separation between the two directions at which the power decreases to one-half of its maximum value, corresponding to $-3$~dB in the normalized power pattern. These characteristics are used to evaluate beam direction, angular coverage, directivity, and side-lobe performance.

% For the metasurface transmitarray lens antenna considered in this thesis, the far-field radiation pattern provides a direct indication of how effectively the lens transforms the spherical wave produced by the feed into a directive outgoing beam. A well-designed phase distribution across the aperture is expected to increase radiation in the desired direction, narrow the main beam, and suppress undesired side lobes. Therefore, the far-field pattern is an important link between the unit-cell phase response, the complete transmitarray aperture, and the antenna gain and coverage observed in the communication-channel evaluation.

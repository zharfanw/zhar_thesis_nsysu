...
This section examines how the lens-assisted MIMO system scales beyond the $9\times9$ configuration of Section~\ref{sec:comparison-antenna-placements} by repeating the same paired Monte Carlo methodology for RX-lens arrays of size $N=3$, $5$, and $9$. All three configurations share an identical room geometry, carrier frequency, bandwidth, transmit power per element, noise figure, and random seed (see the list below); only the array size $N$ and its corresponding set of RX lens angles differ, so that any trend observed across $N$ can be attributed to the array size itself rather than to a change in simulation conditions. Because all TX elements radiate simultaneously within a single ray-traced scene, the $N=9$ data point here reproduces the with-lens results already reported in Section~\ref{sec:comparison-antenna-placements}.

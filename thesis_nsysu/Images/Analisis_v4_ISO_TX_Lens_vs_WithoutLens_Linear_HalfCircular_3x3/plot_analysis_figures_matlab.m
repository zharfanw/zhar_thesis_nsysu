...
for trajectoryIndex = 1:numel(trajectories)
    ...
    for systemIndex = 1:numel(systems)
        ...
        scatter(ax, block.rx_x_m, block.rx_y_m, 45, ...
            ...
            'DisplayName', sprintf('3 RX - %s', systemLabels{systemIndex}));
    end
...
% TX array identik pada semua skenario: 3 elemen ISO, spacing 0.45 m.
txOffsetXmm = zeros(3, 1);
txOffsetYmm = (-0.45:0.45:0.45)' * 1000;
for systemIndex = 1:numel(systems)
    ax = nexttile(t1);
    ...
    scatter(ax, txOffsetXmm, txOffsetYmm, 60, '^', 'filled', ...
        'MarkerFaceColor', [58, 111, 79] / 255, 'DisplayName', '3 TX ISO');
    hold(ax, 'on');
    scatter(ax, rxOffsetXmm, rxOffsetYmm, 55, markers{systemIndex}, ...
        'MarkerEdgeColor', colors(systemIndex, :), ...
        'MarkerFaceColor', colors(systemIndex, :), 'DisplayName', '3 RX');
...
title(t1, {'Trajectory and array geometry: 3 ISO TX x 3 RX', ...
...
metricLabels = {'Combined gain (dB)', 'Mean |rho| off-diagonal', ...
    'Capacity (bit/s/Hz)'};
metricTitles = {'Combined gain: 3 TX', 'TX-branch correlation', ...
...
for panel = 1:3
    ...
    groupedBars(ax, values, 0:2, systemLabels, colors, '%.2f');
    if panel == 1
...
staticLabels = {'Condition number (dB)', 'Effective rank (/3)', ...
...
title(t3, {'Synthetic/combined 3x3 MIMO at the static snapshot', ...
    'Constructed by combining three sequential 3x1 simulations'}, ...
    'FontSize', 14, 'FontWeight', 'bold');
...
channelLabels = {'Mean SNR across 3 RX (dB)', ...
    'Mean capacity across 3 RX (bit/s/Hz)'};
...
title(t4, {'Channel performance along the trajectory', ...
    'Lines = mean across 3 RX; bands = minimum-to-maximum RX range'}, ...
    'FontSize', 14, 'FontWeight', 'bold');
...
mimoLabels = {'Condition number (dB)', 'Effective rank (/3)', ...
...
title(t5, {'Synthetic 3x3 MIMO along the trajectory', ...
    'Each waypoint combines three RX scenarios into a 3x3 matrix'}, ...
    'FontSize', 14, 'FontWeight', 'bold');
...
title(t6, {'Spatial decorrelation: center TX element (index 1) x 3 RX', ...
...
aggregateTitles = {'Median channel magnitude', 'Median link capacity', ...
    'Median MIMO capacity @ 10 dB', 'Median effective rank (/3)', ...
...
title(t8, {'Aggregate comparison: Lens vs Without Lens - 3 ISO TX', ...
...
function validateInputs(summary, waypoint, trajectoryMimo, spatialPooled, ...
    ...
    for trajectoryIndex = 1:numel(trajectories)
        for systemIndex = 1:numel(systems)
            ...
            if nnz(rows) ~= expectedWaypoints(trajectoryIndex) * 3
...
function groupedBars(ax, values, categoryLabels, seriesLabels, colors, numberFormat)
    ...
    for seriesIndex = 1:numel(bars)
        ...
        % Label langsung hanya untuk chart ringkas. Pada 3 kategori x 2

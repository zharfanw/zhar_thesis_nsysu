...
SCENARIOS = [
    common.Scenario(
        "linear_without_lens",
        "Setupv4_20mx20m_wolens_9x9_Patch_iso_randomCom.ipynb",
        "Linear",
        "Without Lens",
    ),
    common.Scenario(
        "half_circular_without_lens",
        "Setupv4_20mx20m_wolens_9x9_Patch_iso_CircularTrajectoryTxCom.ipynb",
        "Half-Circular",
        "Without Lens",
    ),
    common.Scenario(
        "linear_lens",
        "Setupv4_20mx20m_lens_9x9_Patch_iso_TrajectoryTxCom.ipynb",
        "Linear",
        "Lens",
    ),
    common.Scenario(
        "half_circular_lens",
        "Setupv4_20mx20m_lens_9x9_Patch_iso_CircularTrajectoryTxCom.ipynb",
        "Half-Circular",
        "Lens",
    ),
]
...
def parse_static_mimo(cells: list[dict[str, Any]], limit: int) -> dict[str, float]:
    ...
    return {
        ...
        "static_mimo_effective_rank": value(
            r"Effective rank\s*:\s*median\s*=\s*([-+\deE.]+)\s*/\s*9"
        ),
...
def plot_geometry(frames: dict[str, dict[str, Any]]) -> None:
    ...
    for ax, system in zip(axes[1], ["Without Lens", "Lens"]):
        ...
        ax.scatter(tx_offsets[:, 0], tx_offsets[:, 1], color="#3a6f4f", marker="^", s=55, label="9 TX ISO")
        ax.scatter(rx_offsets[:, 0], rx_offsets[:, 1], color=COLORS[system], marker=MARKERS[system], s=48, label="9 RX")
...
def plot_static_rx(frames: dict[str, dict[str, Any]]) -> None:
    ...
    width = 0.38
    metrics = [
        ("combined_gain_db", "Combined gain (dB)", "Combined gain: 9 TX"),
...
def plot_static_mimo(summary: pd.DataFrame) -> None:
    ...
    specs = [
        ...
        ("static_mimo_effective_rank", "Effective rank (/9)", "Effective rank"),
    ...
    fig.suptitle("Synthetic/combined 9×9 MIMO at the static snapshot\nConstructed by combining nine sequential 9×1 simulations", y=1.03, fontsize=14)
    fig.tight_layout()
...
def plot_channel_trajectory(frames: dict[str, dict[str, Any]]) -> None:
    ...
    for row, trajectory in enumerate(["Linear", "Half-Circular"]):
        ...
        axes[row, 0].set(ylabel="Mean SNR across 9 RX (dB)", xlabel="Distance along trajectory (m)", title=f"{trajectory}: SNR")
        axes[row, 1].set(ylabel="Mean capacity across 9 RX (bit/s/Hz)", xlabel="Distance along trajectory (m)", title=f"{trajectory}: capacity")
        axes[row, 1].axhline(1, color="#30343b", lw=1, label="Outage threshold")
        for ax in axes[row]:
            ax.legend(fontsize=8)
    fig.suptitle("Channel performance along the trajectory\nLines = mean across 9 RX; bands = RX minimum–maximum range; notebook logs are rounded", y=1.01, fontsize=14)
    fig.tight_layout()
...
def plot_trajectory_mimo(frames: dict[str, dict[str, Any]]) -> None:
    ...
    fig.suptitle("Synthetic 9×9 MIMO conditioning along the trajectory\nEach waypoint combines nine RX scenarios into a 9×9 matrix", y=1.01, fontsize=14)
    fig.tight_layout()
...
def plot_aggregate(summary: pd.DataFrame) -> None:
    ...
    metrics = [
        ...
        ("trajectory_mimo_erank_median", "Median effective rank (/9)", (0, 9)),
...
def build_report(summary: pd.DataFrame, effect: pd.DataFrame) -> str:
    ...
    return f"""# Analisis 9-TX Isotropik: Lens vs Without Lens pada Trajectory Linear dan Half-Circular
...
**Pada konfigurasi 9-TX ISO ini, Lens tidak meningkatkan median link trajectory.** Median magnitude turun {abs(linear_gain):.2f} dB pada linear dan {abs(half_gain):.2f} dB pada half-circular. Median kapasitas link ikut turun {abs(linear_cap):.2f} dan {abs(half_cap):.2f} bit/s/Hz. Semua skenario tetap memiliki outage 0% pada ambang 1 bit/s/Hz, sehingga outage tidak mampu membedakan desain pada link budget ini.
...
Pada matriks 9×9 snapshot, condition number median membaik dari {v('Linear','Without Lens','static_mimo_condition_db'):.1f} menjadi {v('Linear','Lens','static_mimo_condition_db'):.1f} dB. Namun effective rank turun dari {v('Linear','Without Lens','static_mimo_effective_rank'):.2f} menjadi {v('Linear','Lens','static_mimo_effective_rank'):.2f}, dan capacity @10 dB sedikit turun dari {v('Linear','Without Lens','static_mimo_capacity_10db'):.2f} ke {v('Linear','Lens','static_mimo_capacity_10db'):.2f} bit/s/Hz. Satu condition number saja tidak cukup menilai kualitas multiplexing.
...
Pada half-circular, median kanal adalah {v('Half-Circular','Without Lens','channel_magnitude_median_db'):.2f} dB Without Lens vs {v('Half-Circular','Lens','channel_magnitude_median_db'):.2f} dB Lens; median kapasitas {v('Half-Circular','Without Lens','capacity_median_bits_s_hz'):.2f} vs {v('Half-Circular','Lens','capacity_median_bits_s_hz'):.2f} bit/s/Hz. Gap Lens lebih kecil daripada linear karena lintasan melengkung menyapu sudut datang lebih beragam, tetapi fixed ensemble sembilan Lens angle masih tidak mengungguli patch pada median keseluruhan.
...
Pada linear, median capacity MIMO @10 dB turun dari {v('Linear','Without Lens','trajectory_mimo_capacity_median_10db'):.2f} menjadi {v('Linear','Lens','trajectory_mimo_capacity_median_10db'):.2f} bit/s/Hz dan effective rank turun dari {v('Linear','Without Lens','trajectory_mimo_erank_median'):.2f} menjadi {v('Linear','Lens','trajectory_mimo_erank_median'):.2f}. Half-circular memberi conditioning lebih stabil dan capacity lebih tinggi daripada linear untuk kedua sistem, tetapi Lens tetap lebih rendah: {v('Half-Circular','Lens','trajectory_mimo_capacity_median_10db'):.2f} vs {v('Half-Circular','Without Lens','trajectory_mimo_capacity_median_10db'):.2f} bit/s/Hz.
...
Lens menurunkan pooled decorrelation dari {v('Linear','Without Lens','pooled_decorrelation'):.4f} ke {v('Linear','Lens','pooled_decorrelation'):.4f} pada linear, dan dari {v('Half-Circular','Without Lens','pooled_decorrelation'):.4f} ke {v('Half-Circular','Lens','pooled_decorrelation'):.4f} pada half-circular. Sebaliknya, median-based decorrelation meningkat dari {v('Linear','Without Lens','median_decorrelation'):.4f} ke {v('Linear','Lens','median_decorrelation'):.4f} pada linear dan dari {v('Half-Circular','Without Lens','median_decorrelation'):.4f} ke {v('Half-Circular','Lens','median_decorrelation'):.4f} pada half-circular.
...
def main() -> None:
    ...
    for scenario in SCENARIOS:
        ...
        cfg = item["config"]
        tx_ok = cfg.get("TX_PATTERN_MODE") == "iso" and len(cfg.get("TX_POSITIONS", [])) == 9
        checks.append({"scenario": scenario.key, "check": "nine_iso_tx_invariant", "passed": tx_ok, "detail": f"pattern={cfg.get('TX_PATTERN_MODE')}; tx_count={len(cfg.get('TX_POSITIONS', []))}"})
...
- `simulation_parameters.csv`: parameter konfigurasi dan invariant 9-TX ISO.
...
""",
    ...
    pd.DataFrame(
        [
            ...
            ["03", "Comparison", "Static 9×9 MIMO", "03_static_mimo_comparison.png"],

function fig = plot_vertical_cut_360()
%PLOT_VERTICAL_CUT_360 Display the full 360-degree X-Z-plane cut.

paths = visualization_paths();
dataFile = fullfile(paths.data, "vertical_cut_360_all_patterns.csv");
...
fprintf("Vertical-cut data preview:\n");
disp(data(1:min(20, height(data)), :));

fig = figure( ...
    "Name", "Full 360 Degree Vertical Cut", ...
    "Color", "white", ...
    "Position", [100, 60, 950, 850]);
...
for index = 1:height(scenarioTable)
    ...
    scenarioData = sortrows(data(selection, :), "vertical_angle_deg");
    polarplot( ...
        polarAxis, ...
        deg2rad(scenarioData.vertical_angle_deg), ...
...
polarAxis.ThetaTickLabel = {"+Z", "45°", "+X", "135°", "-Z", "225°", "-X", "315°"};
polarAxis.FontSize = 10;
title(polarAxis, "Full 360° Vertical Cut (X-Z Plane) — All Lens Angles");
legend(polarAxis, "Location", "eastoutside", "Interpreter", "none");

outputFile = fullfile(paths.figures, "matlab_vertical_cut_360_all_patterns.png");
